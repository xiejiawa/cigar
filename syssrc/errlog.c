...
int WriteLog(int loglev, char * filename1, const char * func, int line, char * fmt, ... )
{
    //PbLogLvl = // = getlogLvl();
    if ( loglev >  pbgetlog() ) 
        return -1;
	...
	if(func == NULL)
		sprintf(strLine, "%d|%02d%02d%02d.%03ld|%s:%d",getpid(), tm->tm_hour, tm->tm_min, tm->tm_sec,evt_timestamp.tv_usec/1000, filename,  line);
	else
